...
lwsgs_email_read(struct uv_stream_s *s, ssize_t nread, const uv_buf_t *buf) {
	...
	case LGSSMTP_SENT_DATA:
		n = sprintf(helo, "From: Noreply <%s>\n"
				  ...
				  "%s.\n"
				  "\n.\n",
				  vhd->email_from, vhd->u.username,
				  vhd->u.email, vhd->u.username, vhd->u.ip,
				  vhd->email_confirm_url, vhd->u.token.id,
...
)
{
	...
	case LGSSMTP_IDLE:
		...
		snprintf(s, sizeof(s) - 1,
			 ...
			 " from users where verified = 0 limit 1;");
	...
	case LWS_CALLBACK_HTTP:
		...
		if (!strcmp((const char *)in, "/confirm")) {
			...
			snprintf(s, sizeof(s) - 1,
				 ...
				 "from users where token = '%s';", &cookie[6]);
			...
			snprintf(s, sizeof(s) - 1,
				 ...
				 LWSGS_VERIFIED_ACCEPTED, u.username);
